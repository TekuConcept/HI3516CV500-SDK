...
static unsigned spi_csn[SPI_MAX_NUM] = {1, 0};
#endif
#ifdef __HuaweiLite__
...
int sensor_spi_dev_init(void *pArgs)
{
    ...
    if (pstSpiModuleParam != NULL) {
        ...
        osal_memcpy(sensor, pstSpiModuleParam->cSensor, sizeof(pstSpiModuleParam->cSensor));
    }
    stBusCb.pfn_isp_write_ssp_data = hi_ssp_write;
    if (ckfn_isp_register_bus_callback()) {
        for (i = 0; i < ISP_MAX_DEV; i++) {
            call_isp_register_bus_callback(i, ISP_BUS_TYPE_SSP, &stBusCb);
        }
    } else {
