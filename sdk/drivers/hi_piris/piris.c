...
#endif
{
    ...
    switch (cmd) {
...
#else
            if (read_user_linear_space_valid((HI_U8 *)arg, sizeof(long int)) == HI_FALSE) {
                up(&pstPiris->sem);
                return -EFAULT;
            }
        ...
        case PIRIS_SET_ORGIN:
            if (read_user_linear_space_valid((HI_U8 *)arg, sizeof(PIRIS_DATA_S)) == HI_FALSE) {
                up(&pstPiris->sem);
                return -EFAULT;
            }
        ...
        case PIRIS_GET_STATUS:
            ...
            if (write_user_linear_space_valid((HI_U8 *)arg, sizeof(PIRIS_STATUS_E)) == HI_FALSE) {
                up(&pstPiris->sem);
                return -EFAULT;
            }

            ret = copy_to_user((PIRIS_STATUS_E __user *)arg, &PirisStatus, sizeof(PIRIS_STATUS_E));
            if (ret) {
                up(&pstPiris->sem);
                return -EFAULT;
            }
