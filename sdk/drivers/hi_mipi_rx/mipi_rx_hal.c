...
//#define HI_SUPPORT_SERDES  // when debug thine 241a-242 serdes, need open HI_SUPPORT_SERDES

typedef unsigned int mipi_rx_phy_t;

static hi_uintptr_t mipirx_core_reset_addr;
...
mipi_rx_regs_type_t *g_pstMipirxRegsType = NULL;

unsigned int g_u32MipirxIrqNum = MIPI_RX_IRQ;
...
phy_err_int_cnt_t g_astPhyErrIntCnt[MIPI_RX_MAX_PHY_NUM];
mipi_err_int_cnt_t g_astMipiErrIntCnt[MIPI_RX_MAX_DEV_NUM];
lvds_err_int_cnt_t g_astLvdsErrIntCnt[MIPI_RX_MAX_DEV_NUM];
align_err_int_cnt_t g_astAlignErrIntCnt[MIPI_RX_MAX_DEV_NUM];
...
{
    return &g_pstMipirxRegsType->mipi_rx_phy_cfg[phy_id];
}

static mipi_rx_sys_regs_t *get_mipi_rx_sys_regs(void)
{
    return &g_pstMipirxRegsType->mipi_rx_sys_regs;
}

static  mipi_ctrl_regs_t *get_mipi_ctrl_regs(combo_dev_t devno)
{
    return &g_pstMipirxRegsType->mipi_rx_ctrl_regs[devno].mipi_ctrl_regs;
}

static lvds_ctrl_regs_t *get_lvds_ctrl_regs(combo_dev_t devno)
{
    return &g_pstMipirxRegsType->mipi_rx_ctrl_regs[devno].lvds_ctrl_regs;
}
...
{
    hi_uintptr_t mipi_rx_work_mode_addr;

    mipi_rx_work_mode_addr = (hi_uintptr_t)osal_ioremap(MIPI_RX_WORK_MODE_ADDR, (unsigned long)0x4);
...
void mipi_rx_drv_set_mipi_user_dt(combo_dev_t devno, data_type_t input_data_type, const short data_type[WDR_VC_NUM])
{
    U_MIPI_USERDEF_DT user_def_dt;
    U_MIPI_USER_DEF   user_def;
    volatile mipi_ctrl_regs_t *mipi_ctrl_regs = get_mipi_ctrl_regs(devno);
...
#ifdef HI_SUPPORT_SERDES
    phycfg_mode.bits.phycil0_0_cfg_mode = 0x3;
    phycfg_mode.bits.phycil0_1_cfg_mode = 0x3;
#endif
...
{
    hi_uintptr_t mipi_rx_clock_addr;

    mipi_rx_clock_addr = (hi_uintptr_t)osal_ioremap(MIPI_RX_CRG_ADDR, (unsigned long)0x4);
...
#ifdef HI_SUPPORT_SERDES
extern void serdes_reset_unreset_sensor(unsigned char i2c_dev, int reset);
extern void serdes_enable_disable_sensor_clock(unsigned char i2c_dev, int enable);
#endif

void sensor_enable_disable_clock(sns_clk_source_t sns_clk_source, int enable)
{
    hi_uintptr_t sensor_clock_addr;
    unsigned offset;
    ...
    sensor_clock_addr  = (hi_uintptr_t)osal_ioremap(SNS_CRG_ADDR, (unsigned long)0x4);
...
#ifdef HI_SUPPORT_SERDES
    serdes_enable_disable_sensor_clock(sns_clk_source, enable);
#endif
}
...
{
    set_bit(reset, (combo_dev + 4), mipirx_core_reset_addr);
}
...
#ifdef HI_SUPPORT_SERDES
    serdes_reset_unreset_sensor(sns_reset_source, reset);
#endif
}
...
{
    if (!g_pstMipirxRegsType) {
        g_pstMipirxRegsType = (mipi_rx_regs_type_t *)osal_ioremap(MIPI_RX_REGS_ADDR, (unsigned int)MIPI_RX_REGS_SIZE);
        if (g_pstMipirxRegsType == NULL) {
...
static int mipi_rx_register_irq(void)
{
    ...
    ret = osal_request_irq(g_u32MipirxIrqNum, mipi_rx_interrupt_route, NULL, "MIPI_RX", mipi_rx_interrupt_route);
    if (ret < 0) {
...
{
    osal_free_irq(g_u32MipirxIrqNum, mipi_rx_interrupt_route);
}
...
int mipi_rx_drv_init(void)
{
    ...
    mipirx_core_reset_addr = (hi_uintptr_t)osal_ioremap(MIPI_RX_CRG_ADDR, (unsigned long)0x4);
    if (mipirx_core_reset_addr == 0) {
...
void mipi_rx_drv_exit(void)
{
    ...
    mipi_rx_drv_reg_exit();
    mipi_rx_drv_hw_exit();
    osal_iounmap((void *)mipirx_core_reset_addr);
}
