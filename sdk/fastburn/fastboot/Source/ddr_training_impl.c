...
static int ddr_training_easr(unsigned int base_dmc, unsigned int sref_req)
{
	...
	if (count == 0xffffffff) {
		...
		ddr_training_stat(DDR_ERR_HW_RD_DATAEYE, -1, -1, -1); /* TODO: */
		return -1;
	}
	return 0;
}
/**
 * ddr_training_ctrl_easr
 * @cfg
 *
...
static void ddr_rdqs_sync_rdm(struct ddr_cfg_st *cfg, int offset)
{
	...
	REG_WRITE(rdqnbdl | (rdm << PHY_RDM_BDL_BIT), cfg->cur_phy + DDR_PHY_DXNRDQNBDL2(cfg->rank_idx, cfg->cur_byte));
}
...
static int ddr_dataeye_check_dir(unsigned int direction, unsigned int left,
	unsigned int right, unsigned int *mask, struct ddr_cfg_st *cfg)
{
	int result = 0;
...
static void ddr_dataeye_search_dq(unsigned int left, unsigned int right,
	int *target, unsigned int direction,  struct ddr_cfg_st *cfg)
...
int ddr_dcc_training(struct ddr_cfg_st *cfg)
{
	...
	//DDR_TRINING_BREAK_POINT("save rank rdqs...");
